...
protected:
  ...
  {
    iterator iter =
      std::find_if(this->list().begin(), this->list().end(), Pred(arg1, arg2));
...
private:
  ...
  inline bool validateFileRolling(Level level,
                                  const PreRollOutCallback& preRollOutCallback)
  {
    base::threading::ScopedLock scopedLock(lock());
...
public:
  ...
  {
    return base::utils::Utils::
      installCallback<T, base::type::LoggerRegistrationCallbackPtr>(
  ...
  {
    base::utils::Utils::
      uninstallCallback<T, base::type::LoggerRegistrationCallbackPtr>(
        id, &m_loggerRegistrationCallbacks);
  }
  ...
  {
    return base::utils::Utils::
      callback<T, base::type::LoggerRegistrationCallbackPtr>(
        id, &m_loggerRegistrationCallbacks);
  }
...
public:
  ...
  {
    return base::utils::Utils::
      installCallback<T, base::type::LogDispatchCallbackPtr>(
  ...
  {
    return base::utils::Utils::
      installCallback<T, base::type::PerformanceTrackingCallbackPtr>(
  ...
  {
    return base::utils::Utils::
      callback<T, base::type::PerformanceTrackingCallbackPtr>(
        id, &m_performanceTrackingCallbacks);
  }
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                             \
  ...
  MessageBuilder& operator<<(const QMap<K, V>& map_)
  {
    ...
    int max_ =
      static_cast<int>(base::consts::kMaxLogPerContainer); // to prevent warning
...
#if defined(ELPP_UNICODE)
    std::string s =
      std::string(logger->stream().str().begin(), logger->stream().str().end());
#else
...
#define CPLOG(LEVEL, ...)                                                      \
  C##LEVEL(                                                                    \
    el::base::PErrorWriter, el::base::DispatchAction::NormalLog, __VA_ARGS__)
